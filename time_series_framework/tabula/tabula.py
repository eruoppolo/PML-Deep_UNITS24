...
class Tabula:
    ...
    Attributes:
        ...
        batch_size (int): Batch size used per fine-tuning
    ...
    def __init__(self, llm: str, experiment_dir: str = "trainer_tabula", epochs: int = 100,
                 batch_size: int = 8, categorical_columns: list = [], seq_len: int = 10, stride: int = 5, **train_kwargs):
        """ Initializes Tabula.
        ...
        Args:
            ...
            seq_len: Length of each sequence window for temporal windowing
            stride: Step size to move the window for temporal windowing
    ...
    def fit(self, data: tp.Union[pd.DataFrame, np.ndarray], column_names: tp.Optional[tp.List[str]] = None,
        ...
        # Prepare samples with temporal windowing
        tabula_ds.prepare_samples(seq_len=self.seq_len, stride=self.stride)

        # Print a portion of the preprocessed samples to verify
        print("Sampled preprocessed data with temporal windows:")
        for i, sample in enumerate(tabula_ds.samples[:5]):  # Print the first 5 samples
